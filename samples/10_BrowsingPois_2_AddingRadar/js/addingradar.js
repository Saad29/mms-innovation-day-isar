...
var World = {
  ...
  loadPoisFromJsonData: function loadPoisFromJsonDataFn (poiData) {
    ...
    for (
      ...
      var currentPoi = poiData[currentPlaceNr]
      World.markerDrawableIdle = new AR.ImageResource(
        'assets/' + currentPoi.type + '_idle.png',
        {
          onError: World.onError
        }
      )
      World.markerDrawableSelected = new AR.ImageResource(
        'assets/' + currentPoi.type + '_selected.png',
        {
          onError: World.onError
        }
      )
      var singlePoi = {
        id: currentPoi.id,
        latitude: parseFloat(currentPoi.latitude),
        longitude: parseFloat(currentPoi.longitude),
        altitude: parseFloat(currentPoi.altitude),
  ...
  requestDataFromServer: function requestDataFromServerFn (lat, lon) {
    ...
    var serverUrl =
      ServerInformation.POIDATA_SERVER +
      '?' +
      ServerInformation.POIDATA_SERVER_ARG_LAT +
      '=' +
      lat +
      '&' +
      ServerInformation.POIDATA_SERVER_ARG_LON +
      '=' +
      lon +
      '&' +
      ServerInformation.POIDATA_SERVER_ARG_NR_POIS +
      '=20'
    ...
    xhr.open(
      'GET',
      'https://firebasestorage.googleapis.com/v0/b/mediamarkcodingchallenge.appspot.com/o/json_data.json?alt=media&token=ecdef9a7-2df2-48ce-93e2-cb548a551908',
      true
    )
